...
export default function KesenianPage() {
  ...
  useEffect(() => {
    const fetchKesenian = async () => {
      try {
        const res = await fetch(`${API_URL}/api/kesenian`);
        ...
        setKesenianList(result.data || []);
      } catch (err) {
        console.error("Error fetching kesenian:", err);
        setError(true);
      } finally {
        setLoading(false);
      }
  ...
  const filteredKesenian = kesenianList.filter((item) =>
  ...
  return (
    ...
    >
      <div className="text-center mb-12">
        ...
        >
          <span className="block text-5xl md:text-7xl">KESENIAN</span>
        ...
        <p className="text-biru text-base md:text-lg max-w-3xl mx-auto leading-relaxed">
          Jelajahi berbagai kesenian tradisional dari seluruh penjuru Indonesia
          — tarian, musik, wayang, hingga seni pertunjukan yang penuh makna dan
          keindahan.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto mb-16">
        <div className="relative">
          <input
            type="text"
            placeholder="Cari nama kesenian..."
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {loading ? (
          ...
          <p className="col-span-full text-center text-red-600 text-xl font-bold">
            Gagal memuat data. Pastikan backend berjalan di port 5000!
          </p>
        ) : filteredKesenian.length > 0 ? (
          filteredKesenian.map((item) => (
            <Link
              href={`/jelajahi/kesenian/${item._id}`}
              key={item._id}
